...
if __name__ == "__main__":
    ...
    avrPowerWidget = AvrPowerWidget()
    context = window.avrPower.rootContext()
    context.setContextProperty("avrPowerWidget",avrPowerWidget)
    window.avrPower.setSource(QUrl.fromLocalFile('src/gui/avrPowerRadialBar.qml'))

    throttleWidget = ThrottleWidget()
    context = window.throttle.rootContext()
    context.setContextProperty("throttleWidget",throttleWidget)
    window.throttle.setSource(QUrl.fromLocalFile('src/gui/throttleRadialBar.qml'))
    ...
    timer.timeout.connect(throttleWidget.random_value)
    timer.timeout.connect(avrPowerWidget.random_value)
    timer.start(400)
    window.show()
    sys.exit(app.exec_())